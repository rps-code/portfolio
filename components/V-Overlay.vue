...
<script setup>
  ...
  function leavePageAnim(pageEl, done) {
    ...
    tl.fromTo('.page-overlay__slide__text', { yPercent: 105, autoAlpha: 0.8 }, { yPercent: 0, ease: 'expo.out' }, 0.36)
  }
  ...
  function enterPageAnim(pageEl, done) {
    ...
    tl.from(pageEl, { y: 500, duration: 1, ease: 'power3.out', clearProps: true }, 0.2)
    ...
    tl.fromTo('.page-overlay__slide__text', { yPercent: 0, autoAlpha: 0.8 }, { yPercent: -125, ease: 'expo.out' }, 0)
    ...
    &__slide {
      ...
      &:nth-of-type(1) {
        ...
        @media (prefers-color-scheme: light) {
          ...
          background: #ffe6ed;
          background: linear-gradient(0deg, #ffe6ed 0%, #fff6f8 50%, #ffe6ed 100%);
        }
      }
        ...
        &__circle {
          ...
          @for $i from 1 to 4 {
            &--animate:nth-of-type(#{$i}) {
              animation: infinite 2.5s fade-in-out-#{$i} calc(var(--initial-delay) + calc(var(--circle-animation-offset, 0s)));
            }
          }
        }
      }
